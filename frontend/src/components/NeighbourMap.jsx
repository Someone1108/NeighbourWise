...
const ZONE_GROUPS = {
  ...
  commercialMixed: ['ACZ', 'MUZ', 'C1Z', 'C2Z', 'C3Z', 'B1Z', 'B2Z', 'B3Z', 'B4Z', 'B5Z'],
  industrial: ['IN1Z', 'IN2Z', 'IN3Z'],
  publicSpecial: ['PPRZ', 'PUZ', 'SUZ', 'CDZ', 'DZ', 'PDZ', 'RDZ', 'PCRZ', 'UFZ'],
...
function getZoneGroup(zoneCode) {
  ...
  if (ZONE_GROUPS.growthRuralEnvironmental.includes(code)) return 'growthRuralEnvironmental'

  if (code.startsWith('GRZ') || code.startsWith('NRZ') || code.startsWith('RGZ')) {
    return 'residential'
  }
  if (
    code.startsWith('C') ||
...
export default function NeighbourMap({
  ...
  useEffect(() => {
    ...
    if (zoningLayer && Array.isArray(zoningLayer.features) && zoningLayer.features.length > 0) {
